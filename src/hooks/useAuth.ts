  ...
  const signUp = useCallback(async (email: string, password: string) => {
    ...
    try {
      ...
      if (result.data.user && !result.error) {
        console.log("Sign up successful, creating initial profile");
        ...
        const initialProfileData: any = {
          ...
          updated_at: new Date().toISOString(),
          completed_setup: false // Always false for new users
        };
        ...
        if (storedProfile) {
          ...
          // Map profile data to database fields
          if (parsedProfile.name) initialProfileData.name = parsedProfile.name;
          if (parsedProfile.dateOfBirth) initialProfileData.dob = parsedProfile.dateOfBirth;
          if (parsedProfile.location) initialProfileData.location = parsedProfile.location;
          if (parsedProfile.gender) initialProfileData.gender = parsedProfile.gender;
          if (parsedProfile.workplace) initialProfileData.workplace = parsedProfile.workplace;
          if (parsedProfile.jobTitle) initialProfileData.job_title = parsedProfile.jobTitle;
          if (parsedProfile.education) initialProfileData.education = parsedProfile.education;
          if (parsedProfile.religiousBeliefs) initialProfileData.religious_beliefs = parsedProfile.religiousBeliefs;
          if (parsedProfile.communicationPreferences) initialProfileData.communication_style = parsedProfile.communicationPreferences;
          if (parsedProfile.availability) initialProfileData.availability = parsedProfile.availability;
          if (parsedProfile.supportPreferences) initialProfileData.support_preferences = parsedProfile.supportPreferences;
          if (parsedProfile.journeyNote) initialProfileData.journey_note = parsedProfile.journeyNote;
          if (parsedProfile.supportType) initialProfileData.support_type = parsedProfile.supportType;
          // Ignore completedSetup from localStorage for new users
        }
        
        // Add support type if available
        if (supportType) {
          initialProfileData.support_type = supportType;
        }
        
        // Add journey note if available
        if (journeyNote) {
          initialProfileData.journey_note = journeyNote;
        }
        ...
        if (selectedJourneysJSON) {
          try {
            const selectedJourneys = JSON.parse(selectedJourneysJSON);
            if (Array.isArray(selectedJourneys) && selectedJourneys.length > 0) {
              initialProfileData.support_preferences = selectedJourneys;
            }
          } catch (e) {
            console.error("Error parsing selected journeys:", e);
          }
